...
import { Building2, CheckCircle2, AlertCircle, ExternalLink, LayoutDashboard, Globe, Copy, Check, Clock, UploadCloud } from 'lucide-react';
...
export default function ProvisioningStep() {
  ...
  const [errorDetails, setErrorDetails] = useState<{ msg: string; step: number }[]>([]);
    ...
    async function provision() {
      ...
      if (!result.success) {
        ...
        const errorDetails = result.error.issues.map((issue) => {
          ...
          if (path.includes("hotelIdentity.name")) return { msg: "Falta el nombre del hotel", step: 1 };
          if (path.includes("hotelIdentity.city")) return { msg: "Falta la ciudad o municipio", step: 1 };
          if (path.includes("hotelIdentity.location")) return { msg: "Falta la zona o vereda", step: 1 };
          if (path.includes("rooms") && path.includes("price")) return { msg: "Una habitación no tiene precio definido", step: 3 };
          if (path.includes("rooms") && path.includes("name")) return { msg: "Una habitación no tiene nombre", step: 3 };
          if (path.includes("checkInTime")) return { msg: "Falta el horario de llegada", step: 4 };
          if (path.includes("checkOutTime")) return { msg: "Falta el horario de salida", step: 4 };
          if (path.includes("galleryImages")) return { msg: "Se requieren al menos 3 fotos de la propiedad", step: 2 };
  ...
  if (status === 'error') {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-12 space-y-8 max-w-lg mx-auto">
        <AlertCircle className="mx-auto text-rose-500" size={48} />
        <div className="space-y-2 text-center">
          <h3 className="text-xl font-bold text-white">No se pudo activar la propiedad</h3>
          ...
          {errorDetails.length > 0 ? (
            errorDetails.map((detail, i) => (
              ...
              >
                ...
                <span className="text-zinc-300 text-sm flex-1">{detail.msg}</span>
                <button
                  onClick={() => router.push(`/software/onboarding?step=${detail.step}`)}
            ...
            <div className="p-3 bg-rose-500/10 border border-rose-500/20 rounded-[var(--radius-squircle-lg)]">
              <p className="text-rose-300 text-sm whitespace-pre-line">{errorMessage || t('errorTitle')}</p>
            </div>
          )}
        </div>
