...
export default function SettingsPanel({ initialData, initialStaff = [] }: SettingsPanelProps) {
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'ota' | 'staff' | 'advanced'>('general');
  ...
  const [pendingUndo, setPendingUndo] = useState<{ action: string; data: any } | null>(null);
    ...
    <div className='w-full max-w-7xl mx-auto space-y-[var(--space-pause)] pb-40 font-poppins text-foreground p-8'>
      ...
      <form id="master-settings-form" onSubmit={handleSubmit(onMasterSave)} className="space-y-12">
        <AnimatePresence mode="wait">
            ...
            <motion.div key="gen" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-muted p-10 rounded-[var(--radius-squircle-3xl)] border border-border shadow-xl space-y-8">
                ...
                <div className="space-y-6">
                  ...
                  {/* Tax Regime Selector */}
                  <div>
                    <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2 block">Régimen Tributario</label>
                    <select
          ...
          {activeTab === 'ota' && (
            <motion.div key="ota" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="grid grid-cols-1 lg:grid-cols-3 gap-[var(--space-breath)]">
              ...
              <div className="space-y-[var(--space-breath)]">
                ...
                {showAdvanced && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="glass-card p-[var(--space-breath)] border-amber-500/10 bg-amber-500/5">
          ...
          {showAdvanced && (
            <motion.div key="advanced" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="space-y-8">
      ...
      {showAdvanced && (
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-[var(--space-silence)] pt-[var(--space-pause)] border-t border-red-500/20">
          <div className="flex items-center gap-4 mb-[var(--space-breath)]">
            <div className="p-3 bg-red-500/10 rounded-[var(--radius-squircle-xl)] text-red-500"><AlertOctagon size={28}/></div>
            <div><h3 className="text-red-500 font-bold text-2xl leading-none">Zona de Peligro</h3><p className="text-white/20 text-xs mt-1 uppercase tracking-widest font-bold">Factory Reset</p></div>
          </div>
          <div className="glass-card p-[var(--space-pause)] border-red-500/10 bg-red-500/5 flex flex-col lg:flex-row items-center justify-between gap-[var(--space-breath)] transition-all hover:bg-red-500/[0.08]">
            <div className="max-w-2xl text-center lg:text-left"><h4 className="text-white font-bold text-2xl mb-3">Ejecutar Clean Slate</h4><p className="text-white/50 text-sm leading-relaxed italic">Borra irrevocablemente el historial financiero y las habitaciones para transicionar de demostración a operación real.</p></div>
            <button type="button" onClick={handleCleanSlate} disabled={isCleaning} className="w-full lg:w-auto min-w-[300px] px-12 py-6 bg-red-600/10 hover:bg-red-600 text-red-500 hover:text-white rounded-[var(--radius-squircle-3xl)] font-bold transition-all duration-500 border border-red-500/30 flex items-center justify-center gap-4 shadow-cta disabled:opacity-30">
