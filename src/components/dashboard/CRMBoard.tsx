...
const COLUMNS: { id: Lead['status']; label: string; accent: string }[] = [
  { id: 'new', label: 'Prospectos / Inbox', accent: 'border-border text-muted-foreground' },
...
const CRMBoardPanelView: React.FC<CRMBoardPanelViewProps> = ({
  ...
  return (
    <div className='h-[calc(100vh-8rem)] flex flex-col font-poppins text-foreground'>
      
      {/* HEADER: Liquid Glass Header */}
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4 glass-card p-6 rounded-[var(--radius-squircle-3xl)] border border-border shadow-2xl shadow-black/50 mb-6 ring-1 ring-inset ring-border'>
        <div>
          <h2 className='text-2xl font-bold text-foreground tracking-tight flex items-center gap-3'>
            <TrendingUp className="size-6 text-indigo-400" />
            Pipeline de Ventas
          </h2>
          <p className='text-muted-foreground text-sm mt-1 font-lora'>Gestión topológica de prospectos corporativos y agencias.</p>
        </div>
        <button
          onClick={onOpenModal}
          ...
          {COLUMNS.map((col) => (
            ...
            >
              ...
              <div className={cn('flex justify-between items-center mb-6 pb-3 border-b-2', col.accent)}>
                ...
                <span className='glass-card px-2.5 py-1 text-[10px] font-mono font-bold text-muted-foreground'>
                ...
                <AnimatePresence mode="popLayout">
                  {leads
                    ...
                    .map((lead) => (
                      ...
                      >
                        <div className="absolute -right-4 -top-4 size-16 bg-muted rounded-full blur-2xl group-hover:bg-indigo-500/10 transition-colors"></div>
                        
                        <div className='flex justify-between items-start mb-3 relative z-10'>
                          <h4 className='font-bold text-foreground text-sm leading-tight group-hover:text-indigo-300 transition-colors'>
                            {lead.business_name}
                          </h4>
                        ...
                        {lead.phone && (
                          <div className='text-[11px] text-muted-foreground flex items-center gap-2 mb-3 font-mono'>
                            <Phone size={12} className="text-muted-foreground" /> {lead.phone}
                          </div>
                        )}

                        {lead.notes && (
                          <p className='text-[10px] leading-relaxed text-muted-foreground glass-card p-3 mb-4 line-clamp-2 italic'>
                            "{lead.notes}"
                          </p>
                        )}

                        {/* Acciones de Transición de Estado */}
                        <div className='flex gap-2 pt-3 border-t border-border opacity-0 group-hover:opacity-100 transition-opacity relative z-10'>
                          {col.id !== 'won' && (
                            <button
          ...
          <div className='fixed inset-0 z-[100] bg-black/80 backdrop-blur-xl flex items-center justify-center p-4'>
            ...
            >
              <div className='flex justify-between items-center mb-8'>
                <h3 className='text-2xl font-bold text-foreground tracking-tight'>Nuevo Prospecto</h3>
                <button onClick={onCloseModal} className='text-muted-foreground hover:text-white transition-colors'>
              ...
              <div className='space-y-5'>
                <div className='space-y-2'>
                  <label className='text-[10px] font-bold text-muted-foreground uppercase tracking-widest ml-1'>Identificador / Empresa</label>
                  <input
                    placeholder='Ej: Agencia de Viajes Boyacá'
                    className='w-full p-4 bg-background border border-border rounded-[var(--radius-squircle-2xl)] font-bold text-foreground outline-none focus:ring-2 focus:ring-indigo-500/50 shadow-inner'
                ...
                <div className='space-y-2'>
                  <label className='text-[10px] font-bold text-muted-foreground uppercase tracking-widest ml-1'>Canal de Contacto</label>
                  <input
                    placeholder='Teléfono o WhatsApp'
                    className='w-full p-4 bg-background border border-border rounded-[var(--radius-squircle-2xl)] font-mono text-foreground outline-none focus:ring-2 focus:ring-indigo-500/50 shadow-inner'
                ...
                <div className='space-y-2'>
                  <label className='text-[10px] font-bold text-muted-foreground uppercase tracking-widest ml-1'>Contexto / Notas Forenses</label>
                  <textarea
                    placeholder='Notas iniciales del prospecto...'
                    className='w-full p-4 bg-background border border-border rounded-[var(--radius-squircle-2xl)] text-foreground h-24 resize-none outline-none focus:ring-2 focus:ring-indigo-500/50 shadow-inner'
              ...
              <div className='flex gap-4 mt-10'>
                ...
                <button
                  ...
                  className='flex-[2] py-4 bg-indigo-600 hover:bg-indigo-500 disabled:bg-muted disabled:text-muted-foreground text-white font-bold rounded-[var(--radius-squircle-2xl)] shadow-xl transition-all flex items-center justify-center gap-2'
                >
