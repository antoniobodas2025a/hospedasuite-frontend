...
function buildUrl(
	...
	if (guests !== undefined && guests > 0) {
		p.set("guests", guests.toString());
		p.set("min_capacity", guests.toString());
	} else {
		p.delete("guests");
...
export default function AvailabilitySearchBar({
	...
	const [date, setDate] = useState<DateRange | undefined>(() => {
		...
		if (ci && co) {
			const f = parseISO(ci),
				t = parseISO(co);
			if (isValid(f) && isValid(t)) return { from: f, to: t };
		}
		return undefined;
	});
	...
	const handleConfirmDates = () => {
		if (pendingDate?.from && pendingDate?.to) {
			...
			if (guests > 0)
				pushUrl({
					checkin: pendingDate.from,
					checkout: pendingDate.to,
					guests,
				});
		}
	};
	...
	const handleConfirmGuests = () => {
		...
		if (date?.from && date?.to)
			pushUrl({ checkin: date.from, checkout: date.to, guests: pendingGuests });
	...
	const handleClearAll = () => {
		...
		["checkin", "checkout", "guests", "min_capacity"].forEach((k) =>
			p.delete(k),
		);
				...
				>
					...
					>
						<div className={iconCircle}>
							{isPending ? (
								<Loader2 size={ios} className="text-brand-600 animate-spin" />
							) : date?.from && date?.to ? (
								<CheckCircle2 size={ios} className="text-secondary" />
							) : (
					...
					<div
						className={cn(
							"bg-border mx-2 hidden sm:block",
							sticky ? "h-px w-px h-8" : "h-px w-px h-10",
						)}
					/>
