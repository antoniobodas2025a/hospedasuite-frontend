...
//
// Fetches approved reviews and stats from Supabase on the client.
// Falls back to "be the first" message when no reviews exist.
// Code-split and lazy-loaded via the parent ReviewsSection dynamic wrapper.
// ============================================================================
...
export default function ReviewsSectionContent({ hotelId, hotelName }: ReviewsSectionProps) {
  ...
  useEffect(() => {
    ...
    async function loadReviews() {
      ...
      setReviews(reviewsResult.success ? (reviewsResult.data as Review[]) : []);
      setStats(
        statsResult.success && statsResult.data
          ? (statsResult.data as ReviewStats)
          : defaultStats
      );
      setIsLoading(false);
    }
