...
export default function RoomGallery({ images, roomName, onClose, variant = 'inline', blurDataURL }: RoomGalleryProps) {
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    const oldIndex = galleryImages.findIndex((_, i) => `thumb-${i}` === active.id);
    const newIndex = galleryImages.findIndex((_, i) => `thumb-${i}` === over.id);
      ...
      >
        <div className="space-y-3">
          ...
          {thumbnailItems.length > 0 && (
            <SortableContext items={thumbnailItems.map((_, i) => `thumb-${i + 1}`)} strategy={horizontalListSortingStrategy}>
