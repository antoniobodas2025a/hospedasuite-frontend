								...
								{room.amenities && room.amenities.length > 0 && (
									<GlassCard className="p-5">
										...
										<div className="grid grid-cols-1 xl:grid-cols-2 gap-1">
											{room.amenities.map(
												(
													amenity: string | { id: string; details?: string },
													idx: number,
												) => {
													const id =
														typeof amenity === "string" ? amenity : amenity.id;
													...
													const template = entry.storyTitle
														? {
																...
																story:
																	...
																	t("ota.showcase.premiumService"),
															}
														: {
																icon: entry.icon,
																title: entry.label.toUpperCase(),
																story: t("ota.showcase.premiumService"),
															};
													return (
														<AmenityGlass
															key={idx}
									...
									<div className="space-y-4">
										...
										<div className="flex justify-between items-center pt-3 border-t border-border/40">
											<div>
												...
												<p className="text-sm font-bold text-foreground">
													{defaultGuests}{" "}
													{t("ota.showcase.guest", { count: defaultGuests })}{" "}
													(Capacidad máx: {room.capacity ?? 0})
												</p>
											</div>
