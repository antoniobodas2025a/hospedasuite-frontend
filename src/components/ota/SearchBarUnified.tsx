...
import { Calendar as CalendarIcon, X, Loader2, CheckCircle2, User, ChevronRight, MapPin } from 'lucide-react';
...
interface SearchBarUnifiedProps {
  onSearch?: (filters: { location: string; checkin: string | null; checkout: string | null; guests: number }) => void;
}
...
export default function SearchBarUnified({ onSearch }: SearchBarUnifiedProps) {
  ...
  const pushUrl = useCallback(
    (overrides?: { checkin?: Date; checkout?: Date; guests?: number; location?: string }) => {
  ...
  const handleConfirmDates = () => {
    if (pendingDate?.from && pendingDate?.to) {
      ...
      onSearch?.({ location, checkin: format(pendingDate.from, 'yyyy-MM-dd'), checkout: format(pendingDate.to, 'yyyy-MM-dd'), guests });
    }
  };
  ...
  const handleConfirmGuests = () => {
    ...
    onSearch?.({ location, checkin: date?.from ? format(date.from, 'yyyy-MM-dd') : null, checkout: date?.to ? format(date.to, 'yyyy-MM-dd') : null, guests: pendingGuests });
  };
  ...
  const handleLocationChange = (val: string) => {
    ...
    onSearch?.({ location: val, checkin: date?.from ? format(date.from, 'yyyy-MM-dd') : null, checkout: date?.to ? format(date.to, 'yyyy-MM-dd') : null, guests });
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-0">
        ...
        <div className="flex-1 flex items-center gap-2 px-3 py-2 sm:py-3 sm:rounded-l-full sm:rounded-r-none bg-card border border-border/50 sm:border-r-0 hover:border-border transition-colors cursor-text">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{t('ota.search.destination')}</p>
        ...
        >
          <CalendarIcon size={18} className={cn('shrink-0', date?.from && date?.to ? 'text-secondary' : 'text-muted-foreground/50')} />
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{t('ota.search.stay')}</p>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{t('ota.search.guests')}</p>
            <p className="text-sm text-foreground font-bold truncate">{guestLabel}</p>
          </div>
        </div>
      </div>
