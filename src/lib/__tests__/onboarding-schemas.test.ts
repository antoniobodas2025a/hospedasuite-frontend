...
// ─────────────────────────────────────────────────────────────
function makeValidState(overrides?: Partial<{
...
describe('roomDraftSchema.imageUrls', () => {
  ...
  it('rejects a blob: URL', () => {
    ...
    if (!result.success) {
      ...
      expect(messages.some(m => m.toLowerCase().includes('blob') || m.toLowerCase().includes('inválida') || m.toLowerCase().includes('invalid'))).toBe(true);
    }
  });
...
describe('fullWizardStateSchema.galleryImages', () => {
  it('rejects a blob: URL in galleryImages', () => {
    const state = makeValidState({ galleryImages: ['blob:http://localhost/abc'] });
    const result = fullWizardStateSchema.safeParse(state);
    expect(result.success).toBe(false);
  });

  it('rejects a data: URL in galleryImages', () => {
    const state = makeValidState({ galleryImages: ['data:image/jpeg;base64,abc'] });
    const result = fullWizardStateSchema.safeParse(state);
    expect(result.success).toBe(false);
  });
  ...
  it('accepts valid https:// galleryImages', () => {
    const state = makeValidState({ galleryImages: ['https://cdn.example.com/hotel.webp'] });
    const result = fullWizardStateSchema.safeParse(state);
    expect(result.success).toBe(true);
  });
  ...
  it('rejects blob: URL in room imageUrls (cross-field check)', () => {
    const state = makeValidState({ roomImageUrls: ['blob:http://localhost/room'] });
    const result = fullWizardStateSchema.safeParse(state);
    expect(result.success).toBe(false);
  });
  ...
  it('accepts valid state with multiple rooms and images', () => {
    const state = {
      ...
      galleryImages: ['https://cdn.example.com/1.webp', 'https://cdn.example.com/2.webp'],
      rooms: [
        {
