...
const globalsCss = readFileSync(resolve(PROJECT_ROOT, 'app/globals.css'), 'utf-8');

function readOtaComponent(filename: string): string {
  return readFileSync(resolve(PROJECT_ROOT, 'components/ota', filename), 'utf-8');
}
...
describe('AC1 — OTA Glass Override Variables (T1)', () => {
  ...
  it('glass-light-surface uses oklch(from var(--card) ...)', () => {
    ...
    const surfaceLine = lines.find(l => l.includes('--glass-light-surface:') && !l.includes('-dark'));
