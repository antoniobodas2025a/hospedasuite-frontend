...
describe("S11: No coordinates in any source", () => {
	...
	it("excludes hotel not present in either source", () => {
		const result = resolveHotelCoordinates(["ghost-hotel"], [], []);

		expect(result.size).toBe(0);
	});

	it("excludes hotel with half-null coordinates (lat=null, lng=valid or vice versa)", () => {
		const result = resolveHotelCoordinates(
			["half-null-1", "half-null-2"],
			[],
			[
				{
					hotel_id: "half-null-1",
					id: "loc-1",
					lat: null as unknown as number,
					lng: 5.0,
					precision: "none",
		...
		expect(result.has("half-null-1")).toBe(false);
		expect(result.has("half-null-2")).toBe(false);
	});
});
