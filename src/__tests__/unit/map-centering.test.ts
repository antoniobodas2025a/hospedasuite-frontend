...
import { resolveCenterLocation, handleCenterResult, GeoResult } from '@/lib/map-centering';
...
describe('S2: User feedback when geocoding fails', () => {
  it('returns fly=false and error message for nonexistent place', async () => {
    ...
    const result = await resolveCenterLocation('nonexistent-place', mockGeocode);
  ...
  it('calls onError callback when geocoding returns null', async () => {
    ...
    expect(onError).toHaveBeenCalledWith('No se pudo determinar la ubicación en el mapa');
  });
