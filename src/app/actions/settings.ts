...
export async function saveSettingsAction(settings: HotelSettings) {
	try {
		...
		if (!currentHotel || currentHotel.id !== settings.id) {
			throw new Error(
				"Violación de Seguridad: No tienes permisos sobre este hotel.",
			);
		}
		...
		if (settings.latitude != null && settings.longitude != null) {
			...
			if (locError)
				console.warn("Failed to save manual coordinates:", locError.message);
		}
...
export async function updateHotelProfileAction(hotelId: string, formData: any) {
	try {
		...
		return {
			success: false,
			error: error.message || "Error de validación de datos",
		};
	}
}
