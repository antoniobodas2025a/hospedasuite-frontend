...
export default function CheckoutTerminal({ 
  ...
  return (
    <div className='h-[calc(100vh-8rem)] flex flex-col lg:flex-row gap-6 font-poppins text-zinc-100 pb-4'>
      ...
      <div className='w-full lg:w-80 glass-panel flex flex-col overflow-hidden'>
        <div className='p-6 border-b border-white/5 bg-zinc-950/40'>
          <h3 className='text-xs font-bold uppercase tracking-ultra text-zinc-500'>Ocupación Activa</h3>
        </div>
        <div className='flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar'>
          {bookings.map((booking) => (
            <motion.button
              ...
              className={cn(
                "w-full text-left p-4 rounded-[var(--radius-squircle-2xl)] border transition-all duration-300 relative overflow-hidden group",
        ...
        <AnimatePresence mode='wait'>
          {!selectedBooking ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className='h-full flex flex-col items-center justify-center text-zinc-700'>
              ...
              <p className='text-xs font-bold uppercase tracking-extreme opacity-40'>Aguardando Selección de Nodo</p>
            </motion.div>
          ) : (
            <motion.div key={selectedBooking.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col h-full">
              ...
              <div className='p-8 bg-zinc-900/40 border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-6'>
                <div className='flex items-center gap-5'>
                  <div className='p-4 bg-indigo-500/10 rounded-[var(--radius-squircle-2xl)] border border-indigo-500/20'>
                ...
                <div className='bg-zinc-950/50 p-6 rounded-[var(--radius-squircle-3xl)] border border-white/5 shadow-inner min-w-[240px] text-right'>
              ...
              <div className='flex-1 overflow-y-auto p-8 grid grid-cols-1 xl:grid-cols-2 gap-10 custom-scrollbar'>
                ...
                <div className='space-y-6'>
                  <h4 className='text-[10px] font-bold text-zinc-500 uppercase tracking-ultra flex items-center gap-2 ml-4'>
                    <ArrowRight size={14} className="text-indigo-500" /> Auditoría de Consumos y Alojamiento
                  </h4>

                  <div className='space-y-4'>
                    {/* 1. DESGLOSE DE NOCHES */}
                    <div className='bg-zinc-900/40 border border-white/5 rounded-[var(--radius-squircle-3xl)] overflow-hidden'>
                      <div className='p-5 flex justify-between items-center border-b border-white/5 bg-zinc-950/20'>
                        <div className='flex items-center gap-4'>
                          <div className='p-3 bg-indigo-500/10 text-indigo-400 rounded-[var(--radius-squircle-2xl)]'><BedDouble size={20} /></div>
                          <div>
                    ...
                    {statement?.details.services && statement.details.services.length > 0 ? (
                      <div className='space-y-2'>
                        ...
                        {statement.details.services.map((svc: any) => (
                          <div key={svc.id} className='flex justify-between items-center p-5 bg-zinc-900/20 border border-white/5 rounded-[var(--radius-squircle-3xl)]'>
                            <div className='flex items-center gap-4'>
                              <div className='p-3 bg-amber-500/10 text-amber-400 rounded-[var(--radius-squircle-2xl)]'><Smartphone size={20} /></div>
                              <div>
                      ...
                      <div className='p-8 border border-dashed border-white/5 rounded-[var(--radius-squircle-2xl)] text-center'>
                ...
                <div className='space-y-6'>
                  <div className='bg-zinc-900/40 p-8 rounded-[var(--radius-squircle-3xl)] border border-white/5 shadow-2xl space-y-6'>
                    ...
                    <div className='space-y-5'>
                      <div className='relative'>
                        ...
                        <input type="number" value={paymentForm.amount || ''} onChange={(e) => setPaymentForm({...paymentForm, amount: Number(e.target.value)})}
                          className='w-full pl-11 pr-4 py-5 bg-zinc-950 border border-white/10 rounded-[var(--radius-squircle-2xl)] text-white font-bold text-xl outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all shadow-inner' placeholder="Monto a cobrar"
                        />
                      </div>
                      <div className='grid grid-cols-2 gap-3'>
                        {['cash', 'transfer', 'card', 'wompi'].map((m) => (
                          <button key={m} onClick={() => setPaymentForm({...paymentForm, method: m})}
                            className={cn("py-3 rounded-[var(--radius-squircle-lg)] text-[10px] font-bold uppercase tracking-widest border transition-all",
                      ...
                      {paymentForm.method === 'wompi' && wompiPublicKey ? (
                        ...
                        <button 
                          ...
                          className='w-full py-5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-[var(--radius-squircle-2xl)] font-bold text-sm shadow-xl flex items-center justify-center gap-3 transition-all disabled:opacity-20'
                        >
                          <CheckCircle size={20} />
              ...
              {/* Acción de Cierre */}
              <div className='p-8 border-t border-white/5 flex justify-end gap-6 items-center glass-card !rounded-none'>
                ...
                <button onClick={finalizeCheckout} disabled={isLoading || (statement?.balance || 0) > 1}
                  className={cn("px-10 py-5 rounded-[var(--radius-squircle-2xl)] font-bold text-sm transition-all active:scale-95 shadow-cta flex items-center gap-3",
