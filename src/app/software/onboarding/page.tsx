...
export default function PremiumOnboardingWizard() {
  ...
  if (isLoading && step !== 4) return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center space-y-6">
      ...
      <p className="text-zinc-500 font-mono text-xs uppercase tracking-extreme animate-pulse">Autenticando Nodo</p>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-zinc-100 flex flex-col justify-center py-12 px-4 font-sans relative overflow-hidden selection:bg-indigo-500/30">
      ...
      <div className="max-w-4xl w-full mx-auto relative z-10">
        <div className="text-center mb-12">
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass-card w-16 h-16 flex items-center justify-center mx-auto mb-6 shadow-2xl ring-1 ring-white/5">
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              <motion.div key="step1" variants={slideVariants} initial="enter" animate="center" exit="exit" className="space-y-10">
                <div className="space-y-8 max-w-xl mx-auto">
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-ultra mb-3 ml-2">Nombre Comercial</label>
                    <input type="text" value={hotelName} onChange={(e) => setHotelName(e.target.value)} placeholder="Ej: Villa Secret Stay" className="w-full bg-black/50 border border-white/10 rounded-[var(--radius-squircle-2xl)] p-5 text-white outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all font-medium text-lg placeholder:text-zinc-700" />
                  </div>
                  <label className="flex flex-col items-center justify-center w-full h-56 border-2 border-dashed border-white/10 rounded-[var(--radius-squircle-3xl)] hover:border-indigo-500/40 hover:bg-indigo-500/5 cursor-pointer relative overflow-hidden transition-all group">
                    {logoPreview ? (
            ...
            {step === 2 && (
              <motion.div key="step2" variants={slideVariants} initial="enter" animate="center" exit="exit" className="space-y-8">
                ...
                <div className="space-y-8 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                  {rooms.map((room, idx) => (
                    <motion.div layout key={room.id} className="bg-zinc-900/30 p-6 md:p-8 rounded-[var(--radius-squircle-2xl)] border border-white/5 relative group">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-6">
                          <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-3 text-emerald-400 font-mono bg-emerald-500/10 w-max px-4 py-2 rounded-[var(--radius-squircle-lg)] border border-emerald-500/20">
                          ...
                          <textarea value={room.description} onChange={(e) => updateRoom(room.id, 'description', e.target.value)} placeholder="Describe la experiencia única de este espacio..." className="w-full bg-black/40 border border-white/5 rounded-[var(--radius-squircle-lg)] p-4 text-sm text-zinc-300 outline-none focus:border-indigo-500/50 resize-none h-24 placeholder:text-zinc-700" />
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {Object.values(ROOM_AMENITY_REGISTRY).map(amenity => {
                                ...
                                return (
                                  <button key={amenity.id} onClick={() => toggleAmenity(room.id, amenity.id)} className={`flex items-center gap-2 px-3 py-2 rounded-[var(--radius-squircle-md)] text-xs font-medium transition-all border ${isActive ? 'bg-indigo-500/20 border-indigo-500/50 text-indigo-300' : 'bg-white/5 border-white/5 text-zinc-500 hover:bg-white/10'}`}>
                        ...
                        <div className="space-y-6 border-t md:border-t-0 md:border-l border-white/5 pt-6 md:pt-0 md:pl-8">
                            ...
                            {room.imagePreviews.length > 0 ? (
                              <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                                {room.imagePreviews.map((src, i) => (
                                  <img key={i} src={src} alt="Room" className="h-20 w-28 object-cover rounded-[var(--radius-squircle-lg)] border border-white/10 shrink-0 shadow-lg" />
                                ))}
                                {room.imagePreviews.length < 5 && (
                                  <label className="h-20 w-28 shrink-0 flex items-center justify-center bg-white/5 border border-dashed border-white/10 rounded-[var(--radius-squircle-lg)] cursor-pointer hover:bg-white/10 transition-colors">
                              ...
                              <label className="flex flex-col items-center justify-center w-full h-32 border border-dashed border-white/10 rounded-[var(--radius-squircle-2xl)] hover:border-indigo-500/40 cursor-pointer bg-black/20 group transition-colors">
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="bg-black/40 rounded-[var(--radius-squircle-2xl)] border border-white/5 p-4 flex justify-center date-picker-mac">
                              <DayPicker 
                                mode="range" 
            ...
            {step === 3 && (
              <motion.div key="step3" variants={slideVariants} initial="enter" animate="center" exit="exit" className="text-center py-8 space-y-10">
                ...
                <div className="bg-black/40 p-8 rounded-[var(--radius-squircle-2xl)] border border-white/5 text-left max-w-sm mx-auto shadow-inner">
                    ...
                    <div className="flex gap-4 mt-4">
                      <span className="bg-indigo-500/10 text-indigo-400 text-xs px-3 py-1 rounded-[var(--radius-squircle-md)] border border-indigo-500/20 font-mono">{rooms.length} Nodos</span>
                      <span className="bg-emerald-500/10 text-emerald-400 text-xs px-3 py-1 rounded-[var(--radius-squircle-md)] border border-emerald-500/20 font-bold uppercase tracking-wider">Pionero Activo</span>
                    </div>
                </div>
            ...
            {step === 4 && (
              <motion.div key="step4" variants={slideVariants} initial="enter" animate="center" className="py-24 text-center space-y-8">
                ...
                <div className="space-y-2">
                  <h3 className="text-xl font-bold text-white uppercase tracking-extreme">Compilando Nodo</h3>
