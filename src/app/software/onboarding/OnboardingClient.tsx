...
export default function OnboardingWizard() {
	...
	const handleAuthSuccess = async () => {
		...
		setTimeout(async () => {
			...
			if (user) {
				...
				setError(
					"No se pudo iniciar sesión automáticamente. Por favor intentá de nuevo.",
				);
				setIsLoading(false);
			}
		}, 500);
	...
	const handleNext = () => {
		...
		if (!validation.valid) {
			...
			setValidationErrors({
				[`step-${currentStep}`]: validation.errors.join(", "),
			});
		...
		return (
			<div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center p-8 text-center">
				...
				<button
					...
					className="px-10 py-4 bg-zinc-900 border border-white/10 rounded-[var(--radius-squircle-2xl)] text-white font-bold hover:bg-white/5 transition-all"
				>
					Reintentar
				</button>
			</div>
				...
				<div className="max-w-4xl w-full mx-auto relative z-10">
					<div className="text-center mb-12">
						...
						<p className="text-zinc-500 text-sm mt-2">
							Empezá a configurar tu propiedad
						</p>
					</div>
